...
export default function ProductDetailsModal({
  ...
  const handleDownloadImage = async () => {
    const urls = (product.imageUrls && product.imageUrls.length > 0)
      ? product.imageUrls
      : [(product as any).image || product.imageUrls?.[0] || "https://via.placeholder.com/400"];
    ...
    try {
      ...
      console.error(
        "Error drawing watermarked download canvas, falling back to direct:",
        err,
      );
      try {
        triggerDirectDownload(imgUrl);
        ...
        if (showToast) {
          showToast(
            "اضغط باستمرار على الصورة لحفظها مباشرة في جهازك.",
            "warning",
          );
        }
      }
  ...
  const isOwner =
    currentUserPhone &&
    product.sellerId &&
    normalizePhone(currentUserPhone) === normalizePhone(product.sellerId);
  ...
  const scrollNext = (e: React.MouseEvent) => {
    ...
    if (scrollRef.current) {
      scrollRef.current.scrollBy({
        ...
        behavior: "smooth",
      }); // rtl is negative... wait, if `dir="ltr"` is used for scroll, it's positive. Let's use `clientWidth` and just rely on ltr scrolling.
    }
  };
  ...
  const scrollPrev = (e: React.MouseEvent) => {
    ...
    if (scrollRef.current) {
      scrollRef.current.scrollBy({
        left: scrollRef.current.clientWidth,
        behavior: "smooth",
      }); // for LTR, scrollBy(positive) goes right. Wait, the container has `dir="ltr"`. Let's test left/right carefully inside the component or just check `scrollRef.current.scrollBy({ left: 300 })`. Actually, let's use positive for next and negative for prev since it's dir="ltr".
    }
  };
  ...
  const handleShare = async () => {
    ...
    if (ogImage && product.imageUrls && product.imageUrls[0])
      ogImage.setAttribute("content", product.imageUrls[0]);
  ...
  const handleReport = () => {
    ...
    if (showToast) {
      showToast(
        "تم إرسال بلاغك وسيقوم فريق سوق سند بمراجعته قريباً. شكراً لتعاونك! 🛡️",
        "success",
      );
    } else {
  ...
  const handleAddComment = async () => {
    ...
    const displayName =
      currentUser?.name ||
      (currentUserPhone ? `مستعمل (${currentUserPhone})` : "زائر سوق سند ✦");
    let userRole = "free";
    ...
    try {
      ...
      const drawProductImage = () => {
        return new Promise<void>((resolve) => {
          ...
          img.src = (product as any).image || product.imageUrls?.[0] || "https://via.placeholder.com/400";
        });
      };
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-black/95 overflow-y-auto no-scrollbar scroll-smooth"
      dir="rtl"
    >
      <div className="w-full max-w-lg mx-auto bg-base-black min-h-screen relative flex flex-col border-x border-white/5 shadow-2xl">
        ...
        <div className="relative h-[100dvh] w-full flex flex-col justify-end p-6 overflow-hidden shrink-0">
          ...
          >
            <div
              ref={scrollRef}
              className="flex w-full h-full snap-x snap-mandatory overflow-x-auto no-scrollbar scroll-smooth"
              dir="ltr"
              onScroll={(e) => {
            ...
            {product.imageUrls && product.imageUrls.length > 1 && (
              <>
                <div className="absolute top-1/2 left-4 -translate-y-1/2 z-20">
                  <button
                    onClick={prevImage}
                    className="w-10 h-10 rounded-full bg-black/30 text-white/70 hover:text-white flex items-center justify-center border border-white/10 transition-colors"
                  >
                ...
                <div
                  className="absolute bottom-[40%] left-0 right-0 flex justify-center gap-1.5 z-20 pointer-events-none"
                  dir="ltr"
                >
          ...
          <div className="absolute top-6 left-6 z-20 flex flex-col gap-3">
            <button
              onClick={onClose}
              className="p-3 bg-black border border-[#FFD700]/50 rounded-full text-[#FFD700] hover:bg-[#FFD700]/15 hover:border-[#FFD700] hover:scale-105 active:scale-95 shadow-[0_0_15px_rgba(255,215,0,0.3)] transition-all cursor-pointer"
              title="إغلاق"
            >
              <X className="w-6 h-6 text-[#FFD700]" strokeWidth={2.5} />
            </button>
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="p-3 bg-black/40 hover:bg-black/80 rounded-full text-[#D4AF37] hover:text-[#fcdb71] transition-all scale-100 hover:scale-105 active:scale-95 shadow-lg border border-white/10 cursor-pointer disabled:opacity-50 flex items-center justify-center"
            ...
            <button
              ...
              className="p-3 bg-black/40 hover:bg-black/80 rounded-full text-[#D4AF37] hover:text-[#fcdb71] transition-all scale-100 hover:scale-105 active:scale-95 shadow-lg border border-white/10 cursor-pointer"
              title="إعجاب"
            >
            ...
            <button
              ...
              title="مفضلة"
            >
              <Bookmark className={`w-6 h-6 ${isFavorite ? "fill-[#D4AF37] text-[#D4AF37]" : "text-white"}`} />
            </button>
          </div>
            ...
            <div className="absolute top-6 right-6 z-20 flex gap-2">
              {canEdit && onEdit && (
                <button
                  ...
                  className="p-3 bg-blue-500/80 hover:bg-blue-600 rounded-full text-white transition-all scale-100 hover:scale-105 active:scale-95 shadow-lg border border-blue-500/20"
                >
              ...
              {canDelete &&
                onDelete &&
                (showConfirmDelete ? (
                  <div className="flex items-center gap-2 bg-red-500/95 rounded-full px-4 py-2 border border-red-900 text-white shadow-lg">
                    ...
                    <button
                      ...
                      className="bg-white/10 p-1.5 hover:bg-white/25 rounded-full transition-colors flex items-center gap-1 text-[10px] font-bold"
                    >
                      حذف ✅
                    </button>
                    <button
                      onClick={() => setShowConfirmDelete(false)}
                      className="bg-white/10 p-1.5 hover:bg-white/25 rounded-full transition-colors"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="p-3 bg-red-500/80 hover:bg-red-600 rounded-full text-white transition-all scale-100 hover:scale-105 active:scale-95 shadow-lg border border-red-500/20"
                  >
          ...
          <div className="relative z-10 w-full space-y-4">
            <div className="w-full">
              <h1
                className="text-2xl sm:text-4xl font-extrabold text-white drop-shadow-md leading-normal mb-3 font-display w-full max-w-full line-clamp-2 sm:line-clamp-3 min-h-[3.5rem] sm:min-h-[4.5rem]"
                dir="auto"
                style={{
              ...
              <div className="inline-flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-[#10B981]/25 border border-[#10B981]/30 text-[#10B981] font-black text-xl shadow-md">
            ...
            <div className="flex flex-col items-center justify-center pt-8 pb-2 opacity-85">
              <p className="text-xs text-gray-300 font-medium tracking-wide animate-pulse mb-1">
                اسحب للأعلى لقراءة التفاصيل والتعليقات
              </p>
        ...
        <div className="p-6 pb-48 space-y-8 bg-[#050505]" dir="rtl">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <motion.div animate={isFavorite ? { scale: [1, 1.4, 1] } : {}}>
                <Heart
                  className={`w-4 h-4 transition-colors ${isFavorite ? "text-red-500 fill-red-500" : "text-red-400 fill-transparent"}`}
                />
              </motion.div>
              <span>{product.likes || 0} إعجاب</span>
              {isFavorite && (
                <span className="text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-full mr-1">
                  تم!
                </span>
              )}
            </button>
          ...
          <div className="p-6 rounded-2xl bg-[#0b0b0b] border border-zinc-800/80 shadow-md relative">
            <h3 className="text-lg sm:text-2xl font-black text-white mb-4 flex items-center gap-2">
              <span className="w-1.5 h-5 bg-[#FFD700] rounded-full inline-block"></span>
              تفاصيل الإعلان
            </h3>
            <div className="space-y-4">
              {(product.description || "لا يوجد وصف متاح لهذا الإعلان.")
                ...
                .map((para, index) => (
                  <p
                    key={index}
                    className="text-zinc-100 leading-relaxed text-[15px] sm:text-base break-words font-medium whitespace-pre-wrap"
                    dir="auto"
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-[#0a0a0a] to-[#040404] border border-gray-800 flex flex-col gap-4 shadow-md">
            <div className="flex items-center gap-3.5">
              <div className="relative w-12 h-12 rounded-full overflow-hidden border border-gray-700 shadow-sm shrink-0">
                <img
                  src={
                    product.sellerAvatar || "https://via.placeholder.com/150"
                  }
              ...
              <div className="flex-1 min-w-0 text-right">
                ...
                <p className="text-[11px] text-[#10B981] font-bold mt-0.5 tracking-wider">
                  بائع نشط في سوق سند
                </p>
                <div className="flex items-center gap-0.5 mt-1">
                  ...
                  <span className="text-[10px] text-gray-400 ml-1 mr-1.5">
                    (4.0)
                  </span>
                </div>
            ...
            <button
              ...
              className="mt-1 flex flex-col items-center justify-center py-2.5 px-4 bg-gradient-to-r from-[#D4AF37]/10 to-[#F5D76E]/5 hover:from-[#D4AF37]/20 hover:to-[#F5D76E]/10 text-[#D4AF37] border border-[#D4AF37]/30 rounded-2xl transition-all cursor-pointer shadow-sm hover:scale-[1.02] active:scale-95 w-full"
            >
          ...
          <div className="pt-4 border-t border-gray-900 space-y-5">
            ...
            <motion.div
              animate={
                isShaking
                  ? { x: [-10, 10, -7, 7, -3, 3, 0], scale: [1, 1.01, 1] }
                  : {}
              }
              ...
              {comments.length > 0 ? (
                comments.map((c, index) => {
                  const canDelete =
                    ...
                    currentUserPhone === c.userId ||
                    currentUserPhone === product.sellerId;
                  ...
                  if (normalizedRole === "admin") {
                    badgeNode = (
                      <span className="inline-flex items-center gap-1 text-[9px] font-black px-2 py-0.5 rounded-full bg-[#181818] text-[#F5D76E] border border-[#D4AF37]/50 shadow-[0_4px_10px_rgba(212,175,55,0.15)] shrink-0">
                    ...
                    badgeNode = (
                      <span className="inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full bg-[#D4AF37]/15 text-[#D4AF37] border border-[#D4AF37]/30 shrink-0">
                        <span>متميز ⚡</span>
                      </span>
                    );
                  }

                  const userInitial = c.userName
                    ...
                    <motion.div
                      ...
                      className={`group/comment p-4 rounded-2xl border transition-all duration-300 flex flex-col gap-2 text-right ${
                        normalizedRole === "admin"
                          ? "bg-[#0b0b0b] border-[#D4AF37]/30 shadow-[0_4px_15px_rgba(212,175,55,0.1)]"
                          : normalizedRole === "seller"
                            ? "bg-[#030a06]/70 border-emerald-500/20"
                            : "bg-[#0B0B0B] border-gray-900/80 hover:border-gray-800"
                      }`}
                    >
                      <div className="flex items-center justify-between gap-2 text-[11px]">
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div
                            className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs shadow-inner shrink-0 ${
                              normalizedRole === "admin"
                      ...
                      <p
                        className="text-gray-300 text-xs sm:text-sm leading-relaxed pr-9 pl-4 whitespace-pre-wrap selection:bg-[#D4AF37]/35 select-text"
                        dir="auto"
                        style={{
                          wordBreak: "break-word",
                          overflowWrap: "anywhere" as any,
                        ...
                        <div className="pr-9 pt-1.5">
                          ...
                          >
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
                              <span className="text-[10px] text-white font-bold px-2 py-1 rounded-md bg-black/60 border border-white/10">
                                اضغط للتكبير 🔍
                              </span>
                            </div>
                          </div>
                ...
                <div className="text-center py-7 text-gray-400 bg-[#070707] rounded-2xl border border-gray-900/60 shadow-inner">
                  ...
                  <p className="text-xs font-bold text-gray-400">
                    لا توجد أسئلة أو تعليقات بعد
                  </p>
                  <p className="text-[10px] text-gray-600 mt-1">
                    كن أول من يطرح استفساراً وسيقوم البائع بالرد عليك فورا!
                  </p>
                </div>
              )}
            </motion.div>
            ...
            {commentImage && (
              <div className="p-2.5 rounded-xl bg-gray-950 border border-gray-900 flex items-center justify-between gap-3 animate-fade-in text-sm text-right">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <span className="text-xs text-[#10B981] font-bold truncate">
                    تم إرفاق صورة بالتعليق بنجاح 👍
                  </span>
                </div>
                <button
                  onClick={() => setCommentImage(null)}
            ...
            <div className="relative pt-1 pb-6">
              ...
              <input
                ...
                placeholder={
                  isCompressingImage
                    ? "جاري تشفير وضغط الصورة المرفقة... ⏳"
                    : "اسأل البائع أو اترك استفساراً..."
                }
              />
              ...
              <div className="absolute left-2.5 bottom-[14.5px] flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={
                    (!commentText.trim() && !commentImage) || isCompressingImage
                  }
      ...
      <AnimatePresence>
        {zoomedImage && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-black/100 flex items-center justify-center p-4 cursor-zoom-out"
          >
            <button
              onClick={() => setZoomedImage(null)}
              className="absolute top-6 right-6 p-3 bg-black border border-[#FFD700]/50 rounded-full text-[#FFD700] hover:bg-[#FFD700]/15 hover:border-[#FFD700] transition-all scale-100 hover:scale-105 shadow-[0_0_15px_rgba(255,215,0,0.3)] cursor-pointer"
            >
              <X className="w-6 h-6 text-[#FFD700]" strokeWidth={2.5} />
            </button>
            <motion.img
            ...
            />
            <div className="absolute bottom-6 inset-x-0 text-center text-xs text-gray-500 font-medium">
              سوق سند ✦ SANAD SOUK
            </div>
          </motion.div>
        )}
